...
- Ethereum mainnet wallet (chain 1) with ETH balance
...
2. **Stake ETH**: Deposit ETH into Lido and receive stETH in your wallet — a preview shows the expected stETH amount first; add `--confirm` to execute. `lido-plugin stake --amount-eth 0.1`
...
4. **Request a withdrawal**: Begin the exit process by burning stETH and minting a WithdrawalNFT that represents your ETH claim — note the NFT ID, you'll need it to claim. Expect a 1–5 day wait. `lido-plugin request-withdrawal --amount 0.1 --confirm`
