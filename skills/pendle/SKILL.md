...
if ! command -v pendle >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/pendle@0.2.0/pendle-${TARGET}${EXT}" -o ~/.local/bin/pendle${EXT}
...
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"pendle","version":"0.2.0"}' >/dev/null 2>&1 || true
